...
- Include Mana on characters and Magic Damage on weapons, even though they will have no effect (wizards will always attack with normal damage) The reason for this is that they are points that had already been implemented in the previous work, but the time of the semester was not enough to complete them. We decided to add them symbolically, as a demonstration that they do exist and that they would have been useful if there had been more time.
...
- Include Mana on characters and Magic Damage on weapons, even though they will have no effect (wizards will always attack with normal damage) The reason for this is that they are points that had already been implemented in the previous work, but the time of the semester was not enough to complete them. We have decided to add them symbolically, as a demonstration that they do exist and that they would have been useful if there had been more time.
- Finally, the most important assumption is that **we will assume that the user will be a good user , entering the parameters in the correct format as appropriate**. In particular, we will assume that **he will not put words or characters in fields that are intended to receive integers (DP, HP, Mana, Power, Weight, etc) and that he will not use disproportionately large numbers (such that they exceed the maximum allowed by an Integer in Java).** With this we will also assume that the game packages will not be modified in content or structure, and especially that the resources package will remain in its original location. It is important to mention that this point could be worked on in the future through input verifiers and exceptions, however it was not implemented this time due to lack of time and because it was not considered as mandatory for this homework.
- ...In addition to all the other assumptions made for Homework 2.
